...
```{r echo=FALSE}
  try(print(ggplot(melt(table(subtt$V3,subtt$V2)), aes(x=Var2, y=value, fill=Var1))+geom_bar(stat = "identity")+xlab("Read Length")+ylab("Count") + guides(fill=guide_legend(title="Modified?")) + scale_fill_viridis(discrete = T, option = "D") + COSMO))
...
```{r echo=FALSE}
  try(print(ggplot(melt(table(subtt$V3,subtt$V4)), aes(x=Var2, y=value, fill=Var1))+geom_bar(stat = "identity")+xlab("Strand")+ylab("Count") + guides(fill=guide_legend(title="Modified?")) + scale_fill_viridis(discrete = T, option = "D") + COSMO))
```
